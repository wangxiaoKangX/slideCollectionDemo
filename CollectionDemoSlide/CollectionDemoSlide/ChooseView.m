...
@interface ChooseView ()<DragCellCollectionViewDataSource, DragCellCollectionViewDelegate>
...
- (void) creatUI
{
    ...
    // 设置每个item的大小
    float cellWidth = floorf((self.bounds.size.width - 50)/3);
    layout.itemSize = CGSizeMake(cellWidth, 40);
    layout.sectionInset = UIEdgeInsetsMake(10, 10, 10, 10);
    
    DragCellCollectionView *mainView = [[DragCellCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:layout];
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section{
    ...
    if (section == 0&&[DataManager shareDataManager].isShowHeaderMessage) {
        size= CGSizeMake(self.bounds.size.width, 60);
    }else {
        size= CGSizeMake(self.bounds.size.width, 25);
    }
    return size;
}
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section{
    CGSize size= CGSizeMake(self.bounds.size.width, 1);
    return size;
}
